...
const DomesticTourManagement = ({ onEdit, onAdd }) => {
  ...
  const loadTours = async () => {
    try {
      ...
      const response = await tourApi.getToursByType('domestic');
  ...
  const handleEdit = (tour) => {
    ...
    navigate(`/tour-noi-dia/editor/${tour.slug}`);
  };

  const handleDeleted = () => {
    navigate('/tour-noi-dia/deleted');
  };
  ...
  const handleAdd = () => {
    navigate('/tour-noi-dia/editor?mode=add');
  };
