...
const HomestayVillaPage = () => {
  ...
  const [activeType, setActiveType] = useState('all'); // 'all', 'homestay', 'villa'
  const [activeLocation, setActiveLocation] = useState('all'); // 'all', 'Hà Nội', 'Đà Nẵng', etc.
  const [locations, setLocations] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  // Fetch homestays và villas khi component mount
  useEffect(() => {
    const fetchAccommodations = async () => {
      ...
      try {
        // Gọi API lấy homestays
        const homestayResponse = await accommodationAPI.getAccommodationsByType('Homestay');
        
        // Gọi API lấy villas
        const villaResponse = await accommodationAPI.getAccommodationsByType('Villa');
        
        const homestays = homestayResponse.success ? (homestayResponse.data || []) : [];
        const villas = villaResponse.success ? (villaResponse.data || []) : [];
        
        // Gộp cả 2 mảng
        const combined = [...homestays, ...villas];
  ...
  const homestayCount = allAccommodations.filter(item => 
    item.type?.toLowerCase() === 'homestay'
  ).length;
  const villaCount = allAccommodations.filter(item => 
    ...
    <div className="hotel-resort container mt-5 mb-5">
      ...
      <nav className="tabs-section mb-3" aria-label="Accommodation type selection">
        <ul className="tabs-container">
          ...
          <li>
            <button
              onClick={() => setActiveType('homestay')}
              className={`tab-button ${activeType === 'homestay' ? 'tab-button--active' : ''}`}
              aria-selected={activeType === 'homestay'}
              role="tab"
            >
              <Home className="tab-icon" aria-hidden="true" />
              <span className="tab-label">Homestay ({homestayCount})</span>
            </button>
          </li>
          <li>
            <button
              onClick={() => setActiveType('villa')}
              className={`tab-button ${activeType === 'villa' ? 'tab-button--active' : ''}`}
              aria-selected={activeType === 'villa'}
              role="tab"
            >
              <Home className="tab-icon" aria-hidden="true" />
              <span className="tab-label">Villa ({villaCount})</span>
            </button>
          </li>
        </ul>
      </nav>
          ...
          {filteredAccommodations.length > 0 ? (
            filteredAccommodations.map((item) => (
              <HotelCard
                ...
                image={item.image || 'https://via.placeholder.com/480x332'}
                name={item.name}
                rating={item.rating || 0}
                reviewCount={item.reviewCount || 0}
                stars={item.stars || 0}
                price={item.price || 'Liên hệ'}
                href={`/homestay-villa/${item.slug}`}
              />
            ))
          ) : (
            <div className="text-center py-5 w-100">
              <p className="text-muted">
                Không tìm thấy {activeType === 'homestay' ? 'homestay' : activeType === 'villa' ? 'villa' : 'chỗ nghỉ'} nào
